...
public class DetectionCam {
  ...
  private final Alert m_fuelDetectedOutsideOfBounds =
      new Alert("Fuel Detected Outside Of Bounds", AlertType.kWarning);
  private final Alert m_connectionAlert =
      new Alert("Detection camera is not connected", AlertType.kWarning);
  ...
  public List<DetectionResult> getLatestDetectionResults() {
    var results =
        m_camera.getAllUnreadResults().stream()
            .map((result) -> new DetectionResult(getFuelsX(result), getFuelsY(result)))
            .toList();
