...
public class Hood extends SubsystemBase{
    ...
    private void configureMotor() {
        ...
        config.MotionMagic.withMotionMagicCruiseVelocity(RPM.of(300).times(kMechanismToMotorGearing));
        config.MotionMagic.withMotionMagicAcceleration(RPM.of(500).per(Seconds).times(kMechanismToMotorGearing));
        ...
        config.SoftwareLimitSwitch.withForwardSoftLimitThreshold(kHighSoftLimit.times(kMechanismToMotorGearing));
        config.SoftwareLimitSwitch.withReverseSoftLimitEnable(true);
        config.SoftwareLimitSwitch.withReverseSoftLimitThreshold(kLowSoftLimit.times(kMechanismToMotorGearing)); 
    ...
    public Command setEncoderAngleCommand(Angle angle) {
        return Commands.runOnce(() -> {m_motor.setPosition(angle.div(kMechanismToMotorGearing)); m_encoder.setPosition(kMechanismToEncoderGearing);});
    }
