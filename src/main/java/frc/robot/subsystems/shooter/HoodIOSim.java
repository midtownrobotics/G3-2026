...
public class HoodIOSim implements HoodIO {
  ...
  // Approximate MOI for a hood (kg*m^2)
  private static final double MOI = 0.005;

  private final DCMotorSim m_sim;
  private final PIDController m_controller = new PIDController(20.0, 0, 0);
  ...
  public void updateInputs(HoodIOInputs inputs) {
    if (m_closedLoop) {
      m_appliedVolts = MathUtil.clamp(
          m_controller.calculate(
              m_sim.getAngularPositionRotations(), m_setpoint.in(Rotations)),
          -12.0,
          12.0);
    }
    ...
    inputs.encoderAbsolutePosition = Rotations.of(m_sim.getAngularPositionRotations());
